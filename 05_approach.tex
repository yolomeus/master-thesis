...
To test this, we conduct the following experiment: First, we generate a set of datasets $D=\{d_1, d_2, \cdots, d_n \}$, each aimed at predicting a property $Y_i$ that, based on traditional ranking methods, can be considered relevant for ranking. Then, for each dataset, we train a probing classifier $\mathcal{P}_i$ on top of the fixed hidden representations $H\lay k \in \R^{N \times d}$ of subject model $\mathcal{M}$. This procedure is repeated at every layer $k$. Finally, we compare the classifier's performance across layers, to get a relative measure of how task-specific information is distributed throughout the model. To further put our measurements into perspective, we also employ a random baseline model $\mathcal{B}$ that is trained for each task separately.
