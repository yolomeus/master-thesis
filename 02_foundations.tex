...
Self-Attention is a special case of attention where input vectors and context vectors stem from the same input sequence. It can be seen as the model attending to a sequence, given the sequence itself as context. The general formulation of self-attention further introduces a third sequence of \ti{value} vectors, resulting in three sequences termed \ti{query}, \ti{key} and \ti{value} vectors, which is an analogy to database lookups.
...
As a consequence, each vector in the resulting sequence becomes an attention weighted sum of the vectors in $V$.

% very strong because based on Q and K, because model can inject context into value vectors, based on the rest of the sequence
% scaling d_k for "look in paper" reason
\todo{complete}
\todo{multihead}
...
They are then used to normalize vector $z^{(k)}$:

\begin{equation}
    \Tilde{z}^{(k)} = \gamma^{(k)} \circ \frac{(z^{(k)} - \mu^{(k)})}{\sigma^{(k)}} + \beta^{(k)}
\end{equation}

\begin{equation}
    \tx{LayerNorm}(z) = \Tilde{z}^{(k)}
\end{equation}

where $\gamma^{(k)}$ and $\beta^{(k)}$ are learned parameter vectors for layer $k$ and $\circ$ denotes the element-wise product. $\gamma^{(k)}$ and $\beta^{(k)}$ allow for learning an additional scale and shift of the normalized distribution if required.
