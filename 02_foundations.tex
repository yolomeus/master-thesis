...
Self-Attention is a special case of attention where input vectors and context vectors stem from the same input sequence. It also introduces another set of vectors and treats these $3$ sets as \ti{query}, \ti{key} and \ti{value}
\todo{todo}
...
\begin{equation}
    \text{FFN}(x) = W^{(1)} \text{ReLU}(W^{(0)} x + b^{(0)}) + b^{(1)}
\end{equation}
