...
Recent work on understanding neural language models has aimed at measuring the extent to which certain information is encoded in their word representations. To achieve this, a \ti{probing} classifier or \ti{probe} is trained to predict certain linguistic properties from the fixed embeddings. The probe's performance is then expected to reflect how well these representations capture said property. By employing multiple probing tasks, the presence of different properties can be tested. For example, by training a probe for part-of-speech tagging, we can test whether part-of-speech tags can be extracted from the representations.
